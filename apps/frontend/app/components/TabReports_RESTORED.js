...
import { Download, RefreshCw, FileText, PlusCircle, AlertTriangle } from 'lucide-react';
...
const TabReports = ({ suppressNotifications = false }) => {
  ...
  const showNotificationIfAllowed = (message, type = 'success') => {
  ...
  const fetchReports = async (showNotification = false) => {
    try {
      ...
      if (filters.dateFrom) {
        url += `&fromDate=${encodeURIComponent(filters.dateFrom)}`;
      }

      if (filters.dateTo) {
        url += `&toDate=${encodeURIComponent(filters.dateTo)}`;
      }
  ...
  const handleGenerateReport = async (e) => {
    ...
    try {
      ...
      const requestData = {
        type: reportForm.type === 'service' ? 'services' : reportForm.type,
      ...
      console.log('Generando reporte con datos:', requestData);
      ...
      if (result.success) {
        showNotificationIfAllowed(result.message || "Report generated successfully");
  ...
  const handleDownload = async (reportId) => {
    try {
      ...
      // For diesel reports, handle CSV download directly
      if (result.reportMeta && result.reportMeta.type === 'diesel') {
        const csvContent = result.csvContent || '';
        const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = result.filename || `diesel-report-${reportId}.csv`;
